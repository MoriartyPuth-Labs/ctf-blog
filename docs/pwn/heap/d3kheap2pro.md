...
1. **Vulnerability Location**:
   Inside `d3kheap2pro_ioctl()` during `D3KHEAP2PRO_OBJ_ALLOC`:
   ```c
...
3. **Credential Zeroing & Syscall Execution**:
   Overwriting `cred` objects zeroes out fields like `cred->fsuid`, `cred->euid`, and `cred->user_ns`.
