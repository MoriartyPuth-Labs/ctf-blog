...
A naive **2-byte partial overwrite** (`0x1616` retaddr → `0x12a6` win) fails: the high nibble of byte 1 (bits 12–15) is randomized by PIE, so it needs a 1-in-16 guess _and_ the right stack alignment. Brute-forcing all 16 nibbles still failed (alignment-dependent crash in `win`).

### The clean line — leak the PIE base in-band

`register_runner()` reads exactly 56 bytes into a 56-byte buffer and stores `base+0x1532` (a pointer to `main`) right after it, then prints the buffer with `%s`. Sending exactly 56 bytes makes `printf` walk off the end and **leak the text pointer** in the _same connection_:
