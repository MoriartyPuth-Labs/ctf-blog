...
> The full protocol client (ARX permute, CTR cipher, sponge MAC, S-box KSA, per-packet counter) is in `solve.py` — pure Python, no external deps.
...
* `solve.py` — protocol client + Bellcore fault + flag decryption
