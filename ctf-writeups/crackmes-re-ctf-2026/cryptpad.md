...
> **Challenge:** "We found this on an old machine. Can you decrypt it?" **Files:** `cryptpad.exe`, `flag.enc`

***

### TL;DR

`cryptpad.exe` is a tiny 32-bit Windows GUI "encrypted notepad." Its self-described _"custom encryption algorythm"_ is really:
...
1. **The XOR layers cancel.** The output is `P ^ ks ^ RC4ks ^ ks`. XOR is commutative, the `ks` terms cancel, leaving `P ^ RC4ks`. → The "custom" cipher is **just RC4** with the 8-byte key. (The pre/post XOR keystream `ks` — including the buggy non-resetting index — is completely irrelevant.)
