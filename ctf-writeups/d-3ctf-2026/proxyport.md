...
> **Category**: Misc
>
> **Flag**: `d3ctf{frp_tcp_fin_state_machine_detection_success}`
...
1. **Standard TCP Teardown Behavior**: In standard TCP, when Client A sends a `FIN` packet (`CloseWrite()`), it only closes the stream from A to B (half-closed state). Client A can still receive data sent from B to A until Server B closes its side.
2. **FRP TCP Forwarding Flaw**: `frp`'s application-layer TCP forwarding logic does not fully implement half-closed TCP states. When a client sends a `FIN`, `frp` treats the connection as closed in **both** directions. As a result, any response data sent by the server after the client's `FIN` is dropped.
3. **Detection Mechanism**: We connect to the target port, send an HTTP request, immediately issue a `CloseWrite()` (FIN), and attempt to read from the socket.
